...
function addToCart(id, qty=1){
  const p=resolveProduct({id}); if(!p)return;
  const cart=getCart();const item=cart.find(x=>x.id===p.id);
  if(item)item.qty+=Math.max(1,Number(qty)||1);else cart.push({id:p.id,qty:Math.max(1,Number(qty)||1),name:p.name,price:p.price,image:p.image,category:p.category});
...
function renderProductGrid(el,items=products){if(el){el.innerHTML=items.map(productCard).join('');document.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>addToCart(b.dataset.add))}}
function renderCart(){
  const list=document.getElementById('cartItems');if(!list)return;const cart=getCart();saveCart(cart);
